...
from .forms import AdsForm, RespondForm, ResponsesFilterForm
...
class EditPost(PermissionRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        author = Ads.objects.get(pk=self.kwargs.get('pk')).author.username
...
class Responses(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        """
        if self.kwargs.get('pk') and Ads.objects.filter(id=self.kwargs.get('pk')).exists():
            title = str(Ads.objects.get(id=self.kwargs.get('pk')).title)
            print(title)
        context['form'] = ResponsesFilterForm(
            self.request.user, initial={'title': title})
        context['title'] = title
        if title:
            post_id = Ads.objects.get(title=title)
            context['filter_responses'] = list(Response.objects.filter(
                post_id=post_id).order_by('-dateCreation'))
            context['response_post_id'] = post_id.id
        else:
            context['filter_responses'] = list(Response.objects.filter(
                post_id__author_id=self.request.user).order_by('-dateCreation'))
        context['myresponses'] = list(Response.objects.filter(
            author_id=self.request.user).order_by('-dateCreation'))
        return context
...
class Respond(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        respond.player = User.objects.get(id=self.request.user.id)
        respond.ads = Ads.objects.get(id=self.kwargs.get('pk'))
        respond.save()
